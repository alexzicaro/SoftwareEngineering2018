...
import edu.nd.se2018.homework.hwk5.model.vehicles.CarFactory;
...
public class Road {
	...
	public void assignGate(CrossingGate gate, CrossingGate otherGate){
		gates.add(gate);
		this.otherGate = otherGate;
		if (carFactory != null)
			carFactory = new CarFactory(direction, new Point(startX-roadSize/2,startY), gates, otherGate);  // allows additional gates.  Needs fixing
	}
	
	public void addCarFactory(){
		if (carFactory == null) // We only allow one
			carFactory = new CarFactory(direction, new Point(startX-roadSize/2,startY), gates, otherGate);
	}
